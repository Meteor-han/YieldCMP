...
class Finetuner:
    ...
    def run_BH_reactant(self):
        ...
        for seed in range(min(self.args.repeat, 5)):
            ...
            training_data = []
            test_data = []
            for j in train_test_idxs["pyridyl"]["train_idx"]:
                training_data.append(dataset_BH[j])
            for j in train_test_idxs["pyridyl"]["test_idx"]:
    ...
    def run_SM_ligand(self):
        ...
        (name, start, end) = name_split_dict["SM_test3"]
        self.args.name = name
        all_best_p = []
        # 5 random initialization
    ...
    def run_ELN(self):
        with open(os.path.join("/amax/data/shirunhan/reaction/data/ELN", "raw", "az_reactions_data.json"), "r") as f:
            raw_data_az_BH = json.load(f)
        dataset_az_BH = generate_az_bh_rxns(raw_data_az_BH)
        with open(os.path.join("/amax/data/shirunhan/reaction/data/ELN", "processed-0", "train_test_idxs.pickle"), "rb") as f:
            train_test_idxs = pickle.load(f)
...
if __name__ == '__main__':
    ...
    if runner.args.ds in ["BH", "SM"]:
        runner.run_BH_or_SM()
    elif runner.args.ds in ["BH_oos"]:
        runner.run_BH_reactant()
    elif runner.args.ds in ["SM_oos"]:
        runner.run_SM_ligand()
    elif runner.args.ds in ["ELN"]:
        runner.run_ELN()
    else:
